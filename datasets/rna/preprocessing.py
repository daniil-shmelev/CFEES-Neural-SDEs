...
def compute_chain_torsions(
    ...
    """Return ``(angles, res_ids)`` for valid interior residues only.

    ``angles`` is ``(n_valid, 7)`` in radians, ordered as
    ``(alpha, beta, gamma, delta, epsilon, zeta, chi)``. Residues whose
    res_ids do not form a contiguous run, residues missing any required atom,
    and the leading/trailing residues for which alpha/beta or zeta are
    undefined are dropped. ``res_ids`` records the kept residue numbers.
    """
    ...
    if not angles:
        return np.zeros((0, NUM_ANGLES), dtype=np.float32), np.zeros((0,), dtype=np.int64)

    angles_arr = np.asarray(angles, dtype=np.float32)
    res_ids_arr = np.asarray(kept_res_ids, dtype=np.int64)
    return angles_arr, res_ids_arr


def split_into_contiguous_runs(
    angles: np.ndarray, res_ids: np.ndarray
) -> list[np.ndarray]:
    """Split ``angles`` into runs along monotonically-spaced ``res_ids``."""
    if len(angles) == 0:
        return []
    ...
    for i in range(1, len(res_ids)):
        if res_ids[i] - res_ids[i - 1] != 1:
            runs.append(angles[start:i])
            start = i
    runs.append(angles[start:])
    return runs
