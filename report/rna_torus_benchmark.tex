...
in the $(\delta, \chi)$ plane --- the two backbone dihedrals that
separate the canonical sugar pucker / glycosidic bond modes of
structured RNA. Both populations sit in the same
...
under-predicts the tails, consistent with the per-angle MAE bar
chart in the right panel. Across all seven angles the model beats
the uniform-prior baseline of $\pi/2$ by a factor of $3$--$6\times$,
with $\delta$, $\epsilon$, $\chi$ (the angles constrained by the
dominant sugar pucker and glycosidic conformation) predicted best.

\begin{figure}[hbt]
\centering
\includegraphics[width=0.98\linewidth]{fig_predictions.pdf}
\caption{Trained CFEES25 torus neural SDE on the RNA test set.
\emph{Left}: joint distribution of the $\delta$ (sugar pucker)
and $\chi$ (glycosidic bond) dihedrals, with true held-out targets
in blue and one-step model predictions in red. Both sit in the same
C$3'$-endo~/~anti cluster of A-form RNA. \emph{Middle}:
$\alpha$/$\gamma$ backbone pair; the model captures the main
gauche band but under-covers the boundary clusters. \emph{Right}:
per-angle wrapped MAE; the $\pi/2$ dashed line is the
uniform-prior baseline (the expected error of a random angle on
$\Torus^1$). The model beats that baseline by $3$--$6\times$ on
every angle.}
