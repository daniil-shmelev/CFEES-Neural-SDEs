...
Training loss is indistinguishable between the two integrators, and
\cfees{} reaches at least $n_{\mathrm{steps}} = 3000$ on the same
model.
...
contaminates the measurement. We compare two
...
This stores one state snapshot per integration step, which is the
naive $\mathcal{O}(n_{\mathrm{steps}})$ activation tape that the
reversible scheme is designed to avoid.
\end{itemize}
Figure~\ref{fig:scaling} plots both modes across
$n_{\mathrm{steps}} \in \{5, 10, 20, 50, 100, 200, 400, 800\}$ on
log--log axes.
...
training step on the torus neural SDE at $d=350$, batch $32$, on
log--log axes. Each point is a fresh Python subprocess reading
...
float$32$ activations to within $2\%$. The log--log presentation
makes the transition visible: at small $n_{\mathrm{steps}}$ the
constant non-tape scratch dominates and the blue curve sits on top
of the red one, then rolls over onto an asymptotic slope of one as
the tape term takes over.}
\label{fig:scaling}
\end{figure}
...
\toprule
$n_{\mathrm{steps}}$ & CFEES25+reversible & CG2+checkpoint-full & ratio \\
\midrule
  $5$ & $\mathbf{46.5}$ & $46.3$ & $1.00\times$ \\
 $10$ & $\mathbf{46.5}$ & $46.5$ & $1.00\times$ \\
 $20$ & $\mathbf{46.2}$ & $46.8$ & $1.01\times$ \\
 $50$ & $\mathbf{46.2}$ & $48.2$ & $1.04\times$ \\
$100$ & $\mathbf{46.6}$ & $50.6$ & $1.09\times$ \\
$200$ & $\mathbf{46.5}$ & $54.6$ & $1.17\times$ \\
$400$ & $\mathbf{46.5}$ & $63.2$ & $1.36\times$ \\
$800$ & $\mathbf{46.5}$ & $80.3$ & $1.73\times$ \\
\bottomrule
\end{tabular}
\end{table}
