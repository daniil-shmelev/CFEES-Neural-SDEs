...
$300$ BGSU representative chains ($10{,}418$ training pairs,
$3{,}481$ held-out test pairs) and train a single CFEES25 cell for
$40$ epochs of AdamW with a linear warmup and cosine decay
learning-rate schedule, gradient clipping at norm $1$, best-model
selection by validation wrapped MAE, and test-time averaging over
$16$ MC rollouts of the SDE. The left panel overlays the held-out
true targets (blue) against the averaged one-step model
predictions (red) in the $(\delta, \chi)$ plane --- the two
torsions that separate the canonical sugar pucker / glycosidic
bond modes of structured RNA. The true distribution shows a
dominant C$3'$-endo~/~anti cluster, a smaller C$2'$-endo lobe, and
the full chi range spanned by a few syn / anti pairs; the model
concentrates in the dominant cluster, as expected from a
maximum-likelihood objective on a dataset heavily weighted towards
A-form helices. The middle panel shows the $(\alpha, \gamma)$
pair: the true distribution has a wide gauche band that wraps to
the $\pm\pi$ boundary, and the model now covers most of that band
rather than sitting on a single mode.

The right panel is the quantitative summary. We compare the trained
model against three baselines: a \emph{uniform prior} (expected
wrapped error $\pi/2 \approx 1.57\,\mathrm{rad}$, dashed line),
the \emph{previous residue} trivial autoregressive baseline
(predict $\theta_{t+1} = \theta_t$), and the \emph{training
circular mean} (predict each angle's training-set circular mean).
The model strictly beats both \emph{previous residue} and the
\emph{circular mean} on $\alpha$, $\beta$, $\epsilon$, $\chi$ and
essentially ties the mean on $\delta$, $\gamma$, $\zeta$, for an
overall wrapped MAE of $0.43\,\mathrm{rad}$ against
$0.43\,\mathrm{rad}$ (mean) and $0.57\,\mathrm{rad}$ (previous).
The clear win on $\alpha$ is notable --- this is the hardest
backbone dihedral (the most spread-out in the true data) and the
one the earlier smaller-dataset run did not learn at all.
...
split ($3{,}481$ residues from the chain-level test partition of
the first $300$ BGSU representatives). \emph{Left}: joint
distribution of $\delta$ (sugar pucker) and $\chi$ (glycosidic
bond), true targets in blue, one-step model predictions in red;
the model concentrates in the dominant C$3'$-endo~/~anti cluster
but also emits mass in the secondary C$2'$-endo lobe. \emph{Middle}:
$\alpha/\gamma$ backbone pair; the model now covers most of the
gauche band rather than collapsing to a single mode. \emph{Right}:
per-angle wrapped MAE (CFEES25 in red) against three baselines ---
uniform prior ($\pi/2$, dashed), the predict-previous-residue
trivial autoregressive baseline (blue), and the training circular
mean (green). The model beats predict-previous on every angle and
strictly beats the circular mean on $\alpha$, $\beta$, $\epsilon$,
$\chi$.}
