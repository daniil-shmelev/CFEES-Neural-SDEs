...
$5{,}760$ held-out test pairs) and train a single CFEES25 cell for
$80$ epochs of AdamW with a linear warmup and cosine decay
learning-rate schedule, gradient clipping at norm $1$, hidden
dimension $256$ and context dimension $128$, best-model selection
by validation wrapped MAE, and test-time averaging over $16$ MC
rollouts of the SDE. Beyond the seven angles themselves, the model
...
Over the full test set of $5{,}760$ residues the model's overall
wrapped MAE is $0.410\,\mathrm{rad}$, against $0.418\,\mathrm{rad}$
for the training circular mean and $0.561\,\mathrm{rad}$ for the
predict-previous-residue baseline. The gap to the circular mean is
small because $\sim\!58\%$ of test residues sit within one radian
of the training centroid in the full $7$-dimensional torsion
space, i.e.\ inside a single A-form rotamer cluster, where any
constant prediction close to that cluster is already nearly
optimal. The interesting regime is the complementary
\emph{non-canonical} fraction: the $42\%$ ($n = 2{,}439$) of
residues whose torsion vector lies more than one radian away from
that centroid. These are the residues where a context-dependent
model can actually add value, and they are the residues the right
panel of Figure~\ref{fig:predictions} isolates. On this subset the
model reaches $0.719\,\mathrm{rad}$ against $0.765\,\mathrm{rad}$
for the circular mean --- a $6.0\%$ relative reduction in wrapped
MAE --- and beats the circular mean on six of the seven angles,
with the largest margin on $\alpha$ ($1.174$ vs $1.412\,\mathrm{rad}$,
$+16.9\%$). The only angle on which the model is slightly behind
the circular mean on this subset is $\zeta$, by $0.8\%$, within
run-to-run training noise. Base conditioning is what moves
$\alpha$ from strictly worse than the circular mean to strictly
ahead of it, both overall and on the non-canonical subset. A
proper scoring rule objective, implemented as
``make\_wrapped\_energy\_score\_loss'' in
\texttt{experiment/rna\_losses.py}, gives a further $10\,\%$
improvement on the bimodal $\delta$ angle at the cost of other
angles' MAE --- the time-memory trade-off of that objective is why
we use wrapped MSE here as the primary configuration.
...
split ($5{,}760$ residues from the chain-level test partition of
...
MAE on the \emph{non-canonical} $42\%$ of test residues --- those
...
model beats the circular mean on six of the seven angles, with
the largest margin on $\alpha$ ($+16.9\%$).}
